...
import { useRoqClient, useContentFindManyWithCount } from 'lib/roq';
...
export function ContentListPage(props: ContentListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'client', 'seo_analysis.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['title.contains', 'body.contains', 'keywords.contains'],
          ...params.filters,
        },
        'content',
      ),
  ...
  const { data, error, isLoading, mutate } = useContentFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.content.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'keywords', header: 'Keywords', accessorKey: 'keywords' },
    {
      id: 'publish_date',
      header: 'Publish Date',
      accessorKey: 'publish_date',
      cell: ({ row: { original: record } }: any) =>
        record?.publish_date ? format(record?.publish_date, 'dd-MM-yyyy') : '',
    },
    ...
    hasAccess('client', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('seo_analysis', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'seo_analysis',
          header: 'Seo Analysis',
          accessorKey: 'seo_analysis',
          cell: ({ row: { original: record } }: any) => record?._count?.seo_analysis,
        }
      : null,
