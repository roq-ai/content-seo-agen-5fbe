...
function ContentCreatePage() {
  ...
  const handleSubmit = async (values: ContentInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await roqClient.content.create({ data: values as RoqTypes.content });
      resetForm();
  ...
  const formik = useFormik<ContentInterface>({
    initialValues: {
      ...
      body: '',
      keywords: '',
      publish_date: new Date(new Date().toDateString()),
      writer_id: (router.query.writer_id as string) ?? null,
