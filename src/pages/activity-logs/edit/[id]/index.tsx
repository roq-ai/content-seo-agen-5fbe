...
import { useRoqClient, useActivityLogFindFirst } from 'lib/roq';
...
function ActivityLogEditPage() {
  ...
  const { data, error, isLoading, mutate } = useActivityLogFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ActivityLogInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.activity_log.update({
        data: values as RoqTypes.activity_log,
        where: {
          id,
        },
      });
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <TextInput
            error={formik.errors.browser_info}
            label={'Browser Info'}
            props={{
              name: 'browser_info',
