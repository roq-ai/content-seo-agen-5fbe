...
import { useRoqClient, useSeoAnalysisFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<SeoAnalysisInterface, unknown>;

interface SeoAnalysisListPageProps {
...
export function SeoAnalysisListPage(props: SeoAnalysisListPageProps) {
  const {
    filters = {},
    titleProps = {},
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['seo_report.contains'],
          ...params.filters,
        },
        'seo_analysis',
      ),
  ...
  const { data, error, isLoading, mutate } = useSeoAnalysisFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.seo_analysis.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: SeoAnalysisInterface) => {
    if (hasAccess('seo_analysis', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/seo-analyses/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'seo_score', header: 'seo_score', accessorKey: 'seo_score' },
    { id: 'seo_report', header: 'Seo Report', accessorKey: 'seo_report' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/seo-analyses/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('seo_analysis', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/seo-analyses/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('seo_analysis', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="seo_analysis" />
        </Flex>

        {hasAccess('seo_analysis', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/seo-analyses/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
