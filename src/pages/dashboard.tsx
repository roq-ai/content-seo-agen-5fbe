...
import { useRoqPlatformUserProfiles, useContentCount, useSeoAnalysisCount, useActivityLogCount } from 'lib/roq';

import { SeoAnalysisListPage as ShowCaseList } from 'pages/seo-analyses';

function DashboardPage() {
  const { session } = useSession();

  const contentStat = useContentCount();

  const seoAnalysisStat = useSeoAnalysisCount();

  const activityLogStat = useActivityLogCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Contents',
      result: contentStat?.data,
      isLoading: contentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Seo Analyses',
      result: seoAnalysisStat?.data,
      isLoading: seoAnalysisStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Activity Logs',
      result: activityLogStat?.data,
      isLoading: activityLogStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`seo-analyses`} passHref legacyBehavior>
                  <Button
