...
import { useRoqClient, useClientFindManyWithCount } from 'lib/roq';
...
export function ClientListPage(props: ClientListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useClientFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.client.delete({ where: { id } });
      await mutate();
    } catch (error) {
